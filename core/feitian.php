...
class shy_base {
	 ...
	 * 初始化应用程序
	 */
	public static function shuyang_app() {
		return self::load_sys_class('app');
	}
	/**
	 * 加载系统类方法
	...
	public static function load_app_class($classname, $m = '', $initialize = 1) {
		...
		return self::_load_class($classname, 'app'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'classes', $initialize);
	}
	...
	public static function load_app_func($func, $m = '') {
		$m = empty($m) && defined('ROUTE_M') ? ROUTE_M : $m;
		if (empty($m)) return false;
		return self::_load_func($func, 'app'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'functions');
	}
	...
	public static function my_path($filepath) {
		...
		if (file_exists($path['dirname'].DIRECTORY_SEPARATOR.'SHY_'.$path['basename'])) {
			return $path['dirname'].DIRECTORY_SEPARATOR.'SHY_'.$path['basename'];
		} else {
			return false;
		}
	}
