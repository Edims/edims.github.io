...
final class template_cache {
	...
	public function template_compile($module, $template, $style = 'default') {
		...
		$tplfile = $_tpl = SHY_PATH.'templates'.DIRECTORY_SEPARATOR.$style.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		}
		if ($style != 'default' && !file_exists ( $tplfile )) {
			$style = 'default';
			$tplfile = SHY_PATH.'templates'.DIRECTORY_SEPARATOR.'default'.DIRECTORY_SEPARATOR.$module.DIRECTORY_SEPARATOR.$template.'.html';
		}
	...
	public function template_parse($str) {
		...
		$str = preg_replace ( "/\{ssi\s+(.+)\}/", "<?php include_ssi(\\1); ?>", $str );
		...
		$str = preg_replace_callback("/\{edi:(\w+)\s+([^}]+)\}/i", array($this, 'shy_tag_callback'), $str);
		$str = preg_replace_callback("/\{\/edi\}/i", array($this, 'end_shy_tag'), $str);
		$str = preg_replace_callback("/(\\$[a-zA-Z0-9_\[\]\'\"\$\x7f-\xff]+)/s",array($this, 'addquote_array'),$str);
		$str = "<?php defined('IN_SHUYANG') or exit('No permission resources.'); ?>" . $str;
		return $str;
	}
	...
	public static function shy_tag_callback($matches) {
		return self::shy_tag($matches[1],$matches[2], $matches[0]);;
	}
	/**
	* @param $matches 需要转义的数组元素
	* @return bool|mixed
	* author goldeagle at 2018/10/19
	*/
	public function addquote_array($matches){
		if(!is_array($matches)&&!isset($matches[1])) return false;
	 ...
	 */
	public static function shy_tag($op, $data, $html) {
		...
		if (in_array($op,$tools)) {
			switch ($op) {
				...
				case 'xml':
						$str .= '$xml = shy_base::load_sys_class(\'xml\');';
						$str .= '$xml_data = @file_get_contents(\''.$datas['url'].'\');';
						$str .= '$'.$return.' = $xml->xml_unserialize($xml_data);';
					break;
					
				case 'get':
						$str .= 'shy_base::load_sys_class("get_model", "model", 0);';
	 ...
	 * PC标签结束
	 */
	static private function end_shy_tag() {
		return;
	}
